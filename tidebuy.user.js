...
// @author       You
// @match        https://www.tidebuy.com/product/*
// @grant        none
// ==/UserScript==

(function() {
    'use strict';
    var domain = 'http://www.tidebuy.com/';
    var postdomain = 'tidebuy.com';
    var serv = '172.20.3.194:3000';
    var servs = '172.20.3.194:3001';

    var download = function(){
        $('#outdiv').css({'width':'500px','height':'500px'});
        var pid = $('input[id="moID"]').val();
        $('.list_images img').each(function(idx,obj){
            var imgurl = '';
            if(typeof($(obj).attr("oldurl"))!= "undefined"){//有此属性为高清
                imgurl = $(obj).attr('oldurl');
            }else{//否则是普通大图
                imgurl=$(obj).attr('src');
            }
            ...
            $.post("https://"+servs+"/getimg",{domain:postdomain,imgurl:imgurl,pid:pid},function(result){
                console.log(postdomain+imgurl+pid);
                console.log(result);
            });
        });
    };
