...
(function() {
    ...
    var postdomain = 'aliexpress.com';
    var serv = '10.242.195.138:3000';
    var servs = '10.242.195.138:3001';

    var download = function(){
        $$('#outdiv').css({'width':'500px','height':'500px'});
        var pid = window.runParams.data.actionModule.productId;
        var imgs = window.runParams.data.imageModule.imagePathList;
        console.log(imgs);

        $$.each(imgs,function(idx,val){
            var imgurl = val;
            console.log(imgurl);
            ...
            $$.post("https://"+servs+"/getimg",{domain:postdomain,imgurl:imgurl,out:'img'+idx+'.jpg',pid:pid},function(result){
                //no
                console.log(result);
            });
        });
